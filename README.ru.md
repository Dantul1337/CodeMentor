...
> ⚠️ Никогда не коммить реальные `.env`-файлы. Клиенту безопасно раскрывать только `VAPID_PUBLIC_KEY` / `VITE_VAPID_PUBLIC_KEY` — приватный ключ должен оставаться только на сервере.
